...
sub insertKey {
    my $self = shift;
    my ( $r, $sIp, $cIp, $timestamp, $count, $seal, $type, $level ) = @_;
...
sub ipCountMin {
    ...
    my $pop = $r->rpop($data);
    if ($pop) {
        $r->lpush( "min10:$ser" => $pop );
    }
    else {
        $r->lpush( "min10:$ser" => 0 );
    }
    my $list = $r->lrange( "min10:$ser", 0, $end );
    return $list;
}
...
sub ipCountHour {
    ...
    my $pop = $r->rpop($data);
    if ($pop) {
        $r->lpush( "hour:$ser" => $pop );
    }
    else {
        $r->lpush( "hour:$ser" => 0 );
    }
    my $list = $r->lrange( "hour:$ser", 0, $end );
    return $list;
}
...
sub ipCountDay {
    ...
    my $list = $r->lrange( "day:$ser", 0, $end );
    return $list;
}
...
sub realDayDateIp {
    ...
    my $sData;
    my ( $r, $sec ) = @_;
    my $startTime = strftime( "%Y%m%d", localtime( time() - $sec ) );
    my $dataFrom =
      $r->smembers("$startTime");    #统计服务器的各个时间段IP成员
    if (@$dataFrom) {
        for (@$dataFrom) {
...
sub logSetRedis {
    my $self = shift;
    my ( $r, $key, $logType, $sIp, $logtamp ) = @_;
    my $date = strftime( "%Y%m%d%H", localtime($logtamp) );
    my $hour = strftime( "%H",       localtime($logtamp) );
    $r->hmset(
        "logreal:$date:$logType",    #设置key
        $key => $sIp,                #日志插入hash
    );
